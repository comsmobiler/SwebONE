...
namespace SmoONE.Application
{
    ...
    public class UserService:IUserService
    {
        ...
        /// <summary>
        /// 得到第pagenum页所有该仓储类对象的信息
        /// </summary>
        public List<UserDto> GetPageUsers(int pagenum,int pagesize)
        {
            return Mapper.Map<List<User>, List<UserDto>>(_userRepository.GetPageUsers(pagenum,pagesize).ToList());
        }
        /// <summary>
        /// 得到某个部门的所有用户
        /// </summary>
        ...
        public ReturnInfo DeleteUser(string UserID)
        {
            ...
            if (user != null)
            {
                ...
                {
                    _unitOfWork.RegisterClean(user);
                    _unitOfWork.Rollback();
                    sb.Append(ex.Message);
                    RInfo.IsSuccess = false;
                    RInfo.ErrorInfo = sb.ToString();
