...
namespace SmoONE.Application
{
    ...
    public class AttendanceService : IAttendanceService
    {
        ...
        /// <returns></returns>
        string IAttendanceService.GetATIDByATUser(string UserID)
        {
            string atid = "";
            List<AttendanceTemplate> attendances = _AttendanceTemplateRepository.GetCurrentATByUser(UserID).ToList();
            if (attendances != null && attendances.Count > 0)
            {
                atid = attendances[0].AT_ID;
            }
            else
            {
                atid = null;
        ...
        List<ATCDDto> IAttendanceService.GetATCDByATID(string ATID)
        {
            List<ATCDDto> aTCDDtos = Mapper.Map<List<AT_CustomDate>, List<ATCDDto>>(_AT_CustomDateRepository.GetByATID(ATID).ToList());
            return aTCDDtos;
        }
        #endregion
    }
