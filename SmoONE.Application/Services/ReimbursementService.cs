                    ...
                    if (string.IsNullOrEmpty(ValidateInfo))
                    {
                        try
                        {
                            ...
                            foreach (RB_RowsInputDto row in entity.RB_Rows)
                            {
                                ...
                                {
                                    NewIDs.Add(row.R_ID);
                                    RB_Rows rBrow = _rbrowsRepository.GetByID(row.R_ID).FirstOrDefault();
                                    rBrow.R_Amount = row.R_Amount;
                                    rBrow.R_ConsumeDate = row.R_ConsumeDate;
                                    rBrow.R_Note = row.R_Note;
                                    rBrow.R_TypeID = row.R_TypeID;
                                    _unitOfWork.RegisterDirty(rBrow);
                                }
                                else
                                {
                                    CreateRB_Rows(row);
